                      ...
                      >
                        <div className="orders-mobile-card-header">
                          <div className="orders-mobile-card-headings">
                            ...
                            <p className="orders-mobile-card-subtitle">{order.contractCode?.includes('VSO') ? order.contractCode : order.id?.includes('VSO') ? order.id : (order.contractCode || order.id)}</p>
                          </div>
                      ...
                      return (
                        ...
                        >
                          <td style={{ fontWeight: 700, color: order.isWarning ? '#e11d48' : '#0f766e', display: 'flex', alignItems: 'center', gap: '6px', borderBottom: 'none' }}>
                            {order.contractCode?.includes('VSO') ? order.contractCode : order.id?.includes('VSO') ? order.id : (order.contractCode || order.id)}
                            {order.isWarning && (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
                      <div className="orders-detail-pane__header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '12px', marginBottom: '16px', borderBottom: '2px solid #1e293b' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                          <h3 style={{ fontSize: '20px', margin: 0, fontWeight: 700, color: '#111827' }}>Hồ sơ: {selectedOrder.contractCode?.includes('VSO') ? selectedOrder.contractCode : selectedOrder.id?.includes('VSO') ? selectedOrder.id : (selectedOrder.contractCode || selectedOrder.id)}</h3>
                          <button className="ghost-button" title="Copy mã đơn" onClick={() => copyToClipboard(selectedOrder.contractCode?.includes('VSO') ? selectedOrder.contractCode : selectedOrder.id?.includes('VSO') ? selectedOrder.id : (selectedOrder.contractCode || selectedOrder.id), 'Mã đơn')} style={{ padding: '4px', height: 'auto', color: '#64748b' }}><Copy size={14} /></button>
                        </div>
