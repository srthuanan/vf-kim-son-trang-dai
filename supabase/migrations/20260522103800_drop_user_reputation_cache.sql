...
drop trigger if exists trg_sync_reputation_on_hold_event on public.car_hold_activities;

-- Drop functions
drop function if exists public.sync_reputation_on_hold_event();
drop function if exists public.refresh_user_reputation(text);
drop function if exists public.max_hold_from_score(integer, boolean);

-- Drop the table
drop table if exists public.user_reputation_cache cascade;

-- Update the rpc_hold_car function to remove reputation logic
